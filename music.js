...
// 生成音乐播放器JavaScript
function generateMusicScript(host, uuid) {
    return `
    /* ====================== 音乐播放器脚本 ====================== */
    const MUSIC_PLAYLIST_ID = '14148542684';  // 网易云歌单 ID
    const MUSIC_HOST = '${host}';
    const MUSIC_UUID = '${uuid}';
    ...
    async function initMeting() {
        ...
        try {
            ...
            // 使用更新的 API 接口
            const apiUrl = 'https://api.injahow.cn/meting/?server=netease&type=playlist&id=' + MUSIC_PLAYLIST_ID;
...
if (resizeHandle) {
    ...
    document.addEventListener('click', (e) => {
        if (rightMenu && !rightMenu.contains(e.target)) hideRightMenuImmediate();
    });
    ...
    if (menuVolup) menuVolup.addEventListener('click', () => { ensurePlayerAndRun(ap => ap.volume(Math.min((ap.audio.volume || 0.8) + 0.1, 1), true)); hideRightMenuImmediate(); });
    if (menuVoldown) menuVoldown.addEventListener('click', () => { ensurePlayerAndRun(ap => ap.volume(Math.max((ap.audio.volume || 0.2) - 0.1, 0), true)); hideRightMenuImmediate(); });
    ...
    if (menuClose) menuClose.addEventListener('click', () => {
        ...
        if (musicCapsule) musicCapsule.style.display = 'flex';
        hideRightMenuImmediate();
    });
...
// 👇 修改：增加 proxyCheckUrl 参数，并注入音乐播放器
function dashPage(host, uuid, proxyip, subpass, subdomain, converter, env, clientIP, hasAuth, tgState, cfState, add, addApi, addCsv, tgToken, tgId, cfId, cfToken, cfMail, cfKey, sysParams, dashTitle, proxyCheckUrl, dls, echEnabled, echSni, echDns) {
    ...
    // 生成音乐播放器HTML
    const musicHTML = generateMusicHTML();
    const musicCSS = generateMusicCSS();
    const musicScript = generateMusicScript(host, uuid);
                ...
                <div class="card">
                    ...
                    <div style="max-height:300px;overflow-y:auto;border:1px solid var(--border)">
                        <table class="wl-table">
                            ...
                            <tbody id="whitelistBody"><tr><td colspan="2" style="text-align:center">
                            </tbody>
                        </table>
                    </div>
